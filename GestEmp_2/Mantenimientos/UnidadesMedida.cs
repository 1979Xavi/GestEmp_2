	...
	public partial class UnidadesMedida : Form
	{
		...
		private void cargaDatos()
		{
			try
			{
				...
				dtgvDatos.DataSource = BaseDeDatos.ExecuteSelect(true, "Select idUnidadMedida, CodigoUnidadMedida, Descripcion from MantUnidadesMedida");
		...
		private void bAceptar_Click(object sender, EventArgs e)
		{
			try
			{
				...
				if (accionPantalla == "N")
				{
					...
					strSql = "Insert into MantUnidadesMedida (idUnidadMedida, CodigoUnidadMedida, Descripcion, Activo, usuarioCreacion, fechaHoraCreacion) Values ('" +
							guid + "', '" +
							tCodigo.Text + "', '" +
							tDescripcion.Text + "', " +
							"1, 'SYSTEM','" +
				...
				{
					strSql = "Update MantUnidadesMedida Set CodigoUnidadMedida = '" + tCodigo.Text + "', Descripcion = '" + tDescripcion.Text +
					"', UsuarioModificacion = 'SYSTEM', FechaHoraModificacion = '" + DateTime.Now.ToString() + "'" +
					" Where idUnidadMedida = '" + tId.Text + "'";
				}
		...
		private void bSalir_Click(object sender, EventArgs e)
		{
			try
			{
				DialogResult dialogResult = MessageBox.Show("¿Desea salir del Mto. de Unidades de medida?", "Mto. Unidades de medida", MessageBoxButtons.YesNo);
		...
		private void MuestraDetalle(string id)
		{
			try
			{
				...
				tId.Text = id;
				string strSQl = "Select idUnidadMedida, CodigoUnidadMedida, Descripcion" +
					" from MantUnidadesMedida " +
					" where idUnidadMedida = '" + id + "'";
				DataTable dtDetalle = BaseDeDatos.ExecuteSelect(true, strSQl);

				if (dtgvDatos.Rows.Count > 0)
				{
					tCodigo.Text = dtDetalle.Rows[0]["CodigoUnidadMedida"].ToString();
